...
test_that("Correct initialize with different gid_columns works", {
  l1 <- readRDS("testapps/us1-0005.RDS")
  ...
  input <- reactiveValues(foo = "bar")
  map_id <- "leafdown"
  expect_error(Leafdown$new(spdfs, map_id, input, gid_columns = c("ISO2" = "GID_1")), NA)
})

test_that("Initialize with flipped gid_columns works", {
  l1 <- readRDS("testapps/us1-0005.RDS")
  names(l1)[names(l1) == "GID_1"] <- "ISO2"
  l2 <- readRDS("testapps/us2-0005.RDS")
  spdfs <- list(l1, l2)
  ...
  input <- reactiveValues(foo = "bar")
  map_id <- "leafdown"
  expect_error(Leafdown$new(spdfs, map_id, input, gid_columns = c("GID_1" = "ISO2")), NA)
})

test_that("Initialize with wrong gid_columns fails correctly", {
  l1 <- readRDS("testapps/us1-0005.RDS")
  l2 <- readRDS("testapps/us2-0005.RDS")
  spdfs <- list(l1, l2)
  ...
  input <- reactiveValues(foo = "bar")
  map_id <- "leafdown"
  expect_error(Leafdown$new(spdfs, map_id, input, gid_columns = c("ISO" = "GID_1")),
               "The given gid_columns must specify the columns to join the map levels by.")
})
