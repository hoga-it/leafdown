...
extension of the two map level case. (More than two map levels is supported as of `leafdown` version 1.1.)
...
For this app, we will simply generate some random data.
...
In order for `leafdown` to know which shapes of a lower map level belong to the shapes in the upper map level, we
need to specify how the data of the `spdfs` of different map levels need to be joined.

The `join_map_levels_by` argument is a named vector of length `length(spdfs_list) - 1`, whereby

- the first (named) element tells `leafdown` how to join the first and the second map level
- the second (named) element tells `leafdown` how to join the second and the third map level
- and so on.

The name of an element specifies the name of the join column in the upper map level and the actual element the join column 
...
In this example, map levels 1 and 2 can be joined via columns "GID_0". So the name of first element is "GID_0" 
...
We see, that map levels 2 and 3 can be joined by the columns "GID_1".

<br>

This translates into the following we use the `join_map_levels_by` argument:
