...
is identical to the `spdfs` that come from the `raster` package. It is especially 
important that the data of both `spdfs` have a column `GID_1` that indicates 
which subregions correspond to the regions of the first map level.
...
- `join_map_levels_by` (optional): A named vector with the columns to join the map levels by. 
As a Default this is set to `c("GID_1" = "GID_1")`. <br>
If you use 'world' as first level and the individual countries as second level you have to set it to `c("ISO" = "GID_0")`
