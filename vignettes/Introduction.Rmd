...
digraph { 
  rankdir=LR; 
  graph [fontname = "Segoe UI", fontsize = 28, 
         nodesep="2", ranksep="1", color = dimgrey];
  node [fontname = "Segoe UI", fontsize = 22, color = dimgrey];
  edge [fontname = "Segoe UI", fontsize = 22, color = dimgrey];
...
Using the method `$get_current_metadata()` we can retrieve the metadata of the 
current map level.
With metadata we refer to the data that describes the polygons, such as region names, 
region IDs, etc.

```{r}
metadata <- my_leafdown$get_current_metadata()
print(head(metadata))
```
...
We can retrieve the data of reactive regions via the `$curr_sel_data` attribute.
...
whose parents were selected in the upper (previous) map level.
...
Now we can again add new data as described above.
```{r, eval=FALSE}
my_leafdown$drill_down()
data <- my_leafdown$get_current_metadata()
