...
is identical to the `spdfs` that comes from the `raster` package. It is especially 
...
Using the attribute `$curr_metadata` we can retrieve the metadata of the 
current map level.
With metadata we refer to the data that describes the polygons, such as region names, 
region IDs, etc.
The metadata are automatically set with the `spdf_list[[i]]@data` where i is the current
map level.
