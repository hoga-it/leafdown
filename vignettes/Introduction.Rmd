...
Using the method `$get_current_metadata()` we can obtain the metadata of the
...
Now we can add new columns (with data we want to display  on our map) to the existing metadata
e.g. by using a left join.
It is important that the initial metadata of the data.frame retrieved with `$get_current_data()` 
...
After creating our new data set we give this to our `leafdown`object with the `$add_data` method.
```{r}
my_leafdown$add_data(new_data)
```

The current data of `my_leafdown` can be retrieved via the `curr_data` attribute.
```{r}
...
The specified arguments are internally handed over to the 
`[addPolygons](https://rdrr.io/cran/leaflet/man/map-layers.html)` function
of `leaflet`. Therefore attributes like `fillColor` or `opacity` can be specified
just as for a usual `leaflet` map.
